    ...
    try:
        ...
        if os.path.exists(output_file):
            with open(output_file, 'r') as f:
                try:
                    # Validate existing data is a Python list
                    existing_data = eval(f.read())
                    if not isinstance(existing_data, list):
                        log(f"Invalid format detected in {output_file}. Resetting data.")
                        existing_data = []  # Reset to an empty list
                except Exception as e:
                    log(f"Error reading {output_file}: {e}. Resetting data.")
                    existing_data = []  # Reset to an empty list
        else:
            existing_data = []

        # Append the new result if it's a valid number
        if isinstance(distance, (float, int)):
            existing_data.append(distance)
        else:
            log(f"Invalid distance value for {image_path}: {distance}")
